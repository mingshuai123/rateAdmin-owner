<template>
  <div class="servicePage-container">
    <div class="page-title">
        <div class="left"><i class="el-icon-bell" /> 公共文化服务</div>
        <div class="right">
          <el-tooltip placement="bottom">
            <div slot="content">每年提供公共服务文化活动<br/>10次及以上得3分<br/>5次及以上得2分<br/>3次及以上得1分<br/>3次以下得0分</div>
    ...
    <div class="page-content">
      ...
      <div class="pics">
        ...
        <el-dialog :visible.sync="dialogVisible2" width='80%' title="历史图片" style="margin-top: -8vh;">
          <div style="height:800px;overflow:hidden;overflow-y:auto;">
...
export default {
  ...
  created(){
    this.postAxios('boss/upgrade/info',{type:1}).then(res=>{
        if(res.code==0){
          this.info=res.data;
        }
    })
  ...
  methods:{
    history(){
      this.postAxios('boss/upgrade/history',{type:1}).then(res=>{
          if(res.code===0){
    ...
    submit(){
      ...
      this.postAxios('boss/upgrade/add',{type:1,times:this.activtynum,pics:pic}).then(res=>{
        if(res.code==0){
          this.$message({
            type:'success',
            message:'提交成功',
