<template>
  <div class="servicePage-container">
    <div class="page-title">
        <div class="left"><i class="el-icon-bell" /> 赛事活动服务</div>
        <div class="right">
          <el-tooltip placement="bottom">
            <div slot="content">每年提供赛事活动服务<br/>10次及以上得3分<br/>5次及以上得2分<br/>3次及以上得1分<br/>3次以下得0分</div>
...
export default {
  ...
  created(){
    this.postAxios('boss/upgrade/info',{type:3}).then(res=>{
        if(res.code==0){
          this.info=res.data;
        }
    })
  ...
  methods:{
    history(){
      this.postAxios('boss/upgrade/history',{type:3}).then(res=>{
          if(res.code===0){
    ...
    submit(){
      ...
      this.postAxios('boss/upgrade/add',{type:3,times:this.activtynum,pics:pic}).then(res=>{
        if(res.code==0){
          this.$message({
            type:'success',
            message:'提交成功',
