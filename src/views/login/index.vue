  ...
  watch: {
    $route: {
      handler: function(route) {
        if(route.query){
          if(route.query.redirect!='/Myform'&&route.query.redirect!='/edit'&& route.query.redirect!='/Myform2'){
  ...
  methods: {
    ...
    validatePhone(){
            ...
            if(!this.loginForm.mobile) {
                ...
                // return false
            } else if(!/^1[1234567890]\d{9}$/.test(this.loginForm.mobile)) {
                this.errors = {
                phone:"请输入正确是手机号"
            }
                // return false
        ...
        if (valid) {
          ...
          this.$store.dispatch('user/login', this.loginForm).then(res =>{
              if(res.code == 404){
                this.$message({
                  duration:1500,
                  message:'您不是业主，请注册成为业主后登陆。',
                  type:'error'
                })
              }else{
                this.$store.dispatch('user/taskInfo').then(json=>{
                  if(json.code==10005){
                    console.log(res)
                    this.postAxios('boss/info/get',{member_id:res.member_id}).then(res=>{
                      if(res.code==0){
                          var list=[];
                          for(let x=1;x<6;x++){
                              var obj={};
                          ...
                          var num=0;
                          res.data.name!=''?num++:'';
                          res.data.building_flat_img!=''?num++:'';
                          list.length>0?num++:'';
                          num>2?this.$router.push({ path: this.redirect || '/' }):this.$router.push({ path: this.redirect || '/Apply' });
                      }
